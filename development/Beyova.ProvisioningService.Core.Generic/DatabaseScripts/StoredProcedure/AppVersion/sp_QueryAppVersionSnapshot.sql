IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_QueryAppVersionSnapshot]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_QueryAppVersionSnapshot]
GO

CREATE PROCEDURE [dbo].[sp_QueryAppVersionSnapshot](
...
BEGIN    
    ...
    SET @SqlStatement = 'SELECT [Key]
      ...
      ,[LastUpdatedBy]
      ,[State]
    FROM [dbo].[view_AppVersionSnapshot]';
