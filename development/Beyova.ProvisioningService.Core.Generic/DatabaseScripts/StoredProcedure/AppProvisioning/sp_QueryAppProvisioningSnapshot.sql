IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_QueryAppProvisioningSnapshot]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_QueryAppProvisioningSnapshot]
GO

CREATE PROCEDURE [dbo].[sp_QueryAppProvisioningSnapshot](
...
BEGIN    
    ...
    SET @SqlStatement = 'SELECT [Key]
      ...
      ,[LastUpdatedBy]
      ,[State]
    FROM [dbo].[view_AppProvisioningSnapshot]';
